export const regexLib = {
  ...
  password:
    /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,24}$/, // pelo menos 8 caracteres, incluindo pelo menos uma letra maiúscula, uma letra minúscula, um número e um caractere especial, com um máximo de 24 caracteres.
  username: /^[a-zA-Z0-9]{3,18}$/, //pelo menos 3 caracteres e seja composto apenas por letras maiúsculas e minúsculas e números
  fileName: /^[a-zA-Z0-9]{3,64}$/, //pelo menos 3 e no máximo 64 caracteres e seja composto apenas por letras maiúsculas e minúsculas e números
  fileContent:
    /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{1,}$/, //pelo menos 1 caractere
